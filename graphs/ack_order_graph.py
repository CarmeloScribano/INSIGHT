...
new_order_requests = get_trades_by_type(df=df, msg_type="NewOrderRequest").sort_values(by='TimeStamp')
order_acknowledges = df.groupby(['OrderID']).apply(lambda x: x[x["MessageType"] == "NewOrderAcknowledged"])
